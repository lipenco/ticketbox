  ...
  var gestures = {
    ...
    transform: function(event) {
      if (options.transform) {
        ...
        if (_gesture === 'transform' || Math.abs(1 - scale) > options.scale_treshold || Math.abs(rotation) > options.rotation_treshold) {
          ...
          if (_first) _pos.startCenter = _pos.center;
    ...
    tap: function(event) {
      ...
      var is_double_tap = (function() {
        if (_prev_tap_pos && options.tap_double && _prev_gesture == 'tap' && _pos.start && (_touch_start_time - _prev_tap_end_time) < options.tap_max_interval) {
